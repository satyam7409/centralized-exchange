...
import { OrderJobData, OrderJobResult} from "../types.js";
import { getOrderQueue,getQueueEvents} from "../config/redis.js";
...
const createOrder = async (req: Request, res: Response) => {
  const { userId, side, orderType, symbol, price, qty } = req.body;
  console.log("req data",userId,side,orderType,symbol);

  // shape validation only — API has no order book / balance state to check against
  if (!userId || !side || !orderType || !symbol) {
    throw new ApiError(400, "Missing required fields");
  }
  if (!qty || qty <= 0) throw new ApiError(400, "Invalid quantity");
  if (orderType === "limit" && (!price || price <= 0)) {
    throw new ApiError(400, "Limit order requires a valid price");
  }

  const jobData: OrderJobData = { userId, side, orderType, symbol, price, qty };

  const queue = getOrderQueue(symbol);
  const job = await queue.add("place-order", jobData);

  let result: OrderJobResult;
  try {
    result = await job.waitUntilFinished(getQueueEvents(symbol), 10_000);
  } catch (err) {
    throw new ApiError(504, "Matching engine timed out");
  }
  ...
  return res.status(200).json(new ApiResponse(200, { order: result.order }));
};
